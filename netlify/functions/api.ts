...
export default async (req: Request, context: Context) => {
    ...
    try {
        ...
        if (url.searchParams.get("action") === "getTasks") {
            const sheetData = await sheets.spreadsheets.values.get({ spreadsheetId, range: 'Tasks!A:F' });
        ...
        if (url.searchParams.get("action") === "getFinances") {
            const sheetData = await sheets.spreadsheets.values.get({ spreadsheetId, range: 'Finances!A:D' });
            return new Response(JSON.stringify(sheetData.data.values || []), { headers: { "Content-Type": "application/json" } });
        }

        // 3. CREATE TASK (From Web Form)
        if (url.searchParams.get("action") === "createTask" && req.method === "POST") {
            ...
            await sheets.spreadsheets.values.append({
                spreadsheetId, range: 'Tasks!A:F', valueInputOption: 'USER_ENTERED',
                // Pad to 6 columns to match Kanban board: [Name, Status, Category, Time, Added, Deadline]
                requestBody: { values: [[body.task, 'Pending', '', '', now, body.deadline || "-"]] }
            });
        ...
        if (url.searchParams.get("action") === "updateTaskStatus" && req.method === "POST") {
            ...
            await sheets.spreadsheets.values.update({
                spreadsheetId, range: `Tasks!B${body.rowNumber}`, valueInputOption: 'USER_ENTERED',
        ...
        if (url.searchParams.get("action") === "deleteTask" && req.method === "POST") {
            const body = await req.json();
            await sheets.spreadsheets.values.clear({ spreadsheetId, range: `Tasks!A${body.rowNumber}:F${body.rowNumber}` });
            return new Response(JSON.stringify({ success: true }), { headers: { "Content-Type": "application/json" }});
        }
